...
class FFALoraGM(FedBase):
    def __init__(self, args):
        super(FFALoraGM,self).__init__(args)
        self.mask_dict=None
    def run(self):
        args = self.args
        ...
        for r in range(args.num_rounds):
            ...
            for client_id in participants:
                ...
                if self.mask_dict:
                    for name, param in model.named_parameters():
                        if "lora_B" in name:
                            if self.mask_dict[name.replace("default.weight","weight")] == 1:
            ...
            global_dict = self.aggerate_local_models(local_dict_list, global_dict, sample_num_list,r)
        ...
        model.save_pretrained(f"./output/{args.fed_alg}/{args.dataset_name.replace('/','_')+str(args.seed)+'_'+str(self.args.c)+'_'+str(self.args.beta)}")
        tokenizer.save_pretrained(f"./output/{args.fed_alg}/{args.dataset_name.replace('/','_')+str(args.seed)+'_'+str(self.args.c)+'_'+str(self.args.beta)}")
    ...
    def aggerate_local_models(self, local_dict_list, global_dict, sample_num_list, num_round):
        ...
        with torch.no_grad():
            for key in global_dict.keys():
                if "lora_B".lower() in key.lower():
                    avg_dict[key] = sum([local_dict[key] * sample_num_list[idx] / total_samples 
        ...
        if (num_round+1) % 20 ==0:
            ...
            self.mask_dict = fixed_sample_layers(stat_dict, ritio=self.args.c + ((num_round+1)//20)*self.args.beta)
        with torch.no_grad():
            for key in global_dict.keys():
                if "lora_B".lower() in key.lower():
